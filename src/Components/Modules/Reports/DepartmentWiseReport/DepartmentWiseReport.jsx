...
import BizClass from "./DepartmentWiseReport.module.scss";
    ...
    try {
      ...
      if (result.response.responseCode === 1) {
        if (result.response.responseData) {
          if (result.response.responseData && result.response.responseData.items.length > 0) {
            setFilteredTicketHistoryDataList(result.response.responseData.items);
          } else {
  ...
  return (
    <div className={BizClass.PageStart}>
      ...
      <DataGrid rowData={filteredTicketHistoryDataList} loader={isLoadingTicketHistoryDataList ? <Loader /> : false} onGridReady={onGridReady}>
        ...
        <DataGrid.Column field="DepartmentMasterName" headerName="Department" width="260px" />
        <DataGrid.Column field="#" headerName="Sub Department" width="260px" />
        <DataGrid.Column field="Scheduled" headerName="Applied" width="150px" cellStyle={{ "text-align": "right" }} />
        <DataGrid.Column field="#" headerName="Discarded" width="150px" cellStyle={{ "text-align": "right" }} />
        <DataGrid.Column field="Pending" headerName="Pending" width="150px" cellStyle={{ "text-align": "right" }} />
        <DataGrid.Column field="Appointment" headerName="Total Case" width="220px" cellStyle={{ "text-align": "right" }} />
      </DataGrid>
    </div>
  );
}
